(function (global, factory) {
    ...
    function getParSignFlags(m) {
        if (m._pf == null) {
            m._pf = defaultParSignFlags();
        }
        return m._pf;
    }
    ...
    function valid__createInvalid (flags) {
        ...
        if (flags != null) {
            extend(getParSignFlags(m), flags);
        }
        else {
            getParSignFlags(m).userInvalidated = true;
        }
    ...
    addParseToken(['MMM', 'MMMM'], function (input, array, config, token) {
        ...
        if (month != null) {
            ...
            getParSignFlags(config).invalidMonth = input;
        }
    });
    ...
    function checkOverflow (m) {
        ...
        if (a && getParSignFlags(m).overflow === -2) {
            overflow =
            ...
            if (getParSignFlags(m)._overflowDayOfYear && (overflow < YEAR || overflow > DATE)) {
                overflow = DATE;
            }

            getParSignFlags(m).overflow = overflow;
        }
    ...
    function configFromISO(config) {
        ...
        if (match) {
            getParSignFlags(config).iso = true;
    ...
    function configFromArray (config) {
        ...
        if (config._dayOfYear) {
            ...
            if (config._dayOfYear > daysInYear(yearToUse)) {
                getParSignFlags(config)._overflowDayOfYear = true;
            }
    ...
    function configFromStringAndFormat(config) {
        ...
        config._a = [];
        getParSignFlags(config).empty = true;
        ...
        for (i = 0; i < tokens.length; i++) {
            ...
            if (parsedInput) {
                ...
                if (skipped.length > 0) {
                    getParSignFlags(config).unusedInput.push(skipped);
                }
            ...
            if (formatTokenFunctions[token]) {
                if (parsedInput) {
                    getParSignFlags(config).empty = false;
                }
                else {
                    getParSignFlags(config).unusedTokens.push(token);
                }
        ...
        getParSignFlags(config).charsLeftOver = stringLength - totalParsedInputLength;
        if (string.length > 0) {
            getParSignFlags(config).unusedInput.push(string);
        }

        // clear _12h flag if hour is <= 12
        if (getParSignFlags(config).bigHour === true &&
                config._a[HOUR] <= 12 &&
                config._a[HOUR] > 0) {
            getParSignFlags(config).bigHour = undefined;
        }
        // handle meridiem
    ...
    function configFromStringAndArray(config) {
        ...
        if (config._f.length === 0) {
            getParSignFlags(config).invalidFormat = true;
        ...
        for (i = 0; i < config._f.length; i++) {
            ...
            currentScore += getParSignFlags(tempConfig).charsLeftOver;

            //or tokens
            currentScore += getParSignFlags(tempConfig).unusedTokens.length * 10;

            getParSignFlags(tempConfig).score = currentScore;
    ...
    function parSignFlags () {
        return extend({}, getParSignFlags(this));
    }

    function invalidAt () {
        return getParSignFlags(this).overflow;
    }
    ...
    addWeekParseToken(['dd', 'ddd', 'dddd'], function (input, week, config) {
        ...
        if (weekday != null) {
            ...
            getParSignFlags(config).invalidWeekday = input;
        }
    });
    ...
    addParseToken(['h', 'hh'], function (input, array, config) {
        ...
        getParSignFlags(config).bigHour = true;
    });

    // LOCALES

    function localeIsPM (input) {
        // IE8 Quirks Mode & IE7 Standards Mode do not allow accesSign strings like arrays
    ...
    momentPrototype__proto.parSignFlags = parSignFlags;
    ...
    function locale_set__set (config) {
        ...
        // Lenient ordinal parSign accepts just a number in addition to
    ...
    //! moment.js locale configuration
    //! locale : hindi (hi)
    //! author : Mayank Signhal : https://github.com/mayankSignhal
    ...
    function translateSignular(number, withoutSuffix, key, isFuture) {
    ...
    function lt__translate(number, withoutSuffix, key, isFuture) {
        var result = number + ' ';
        if (number === 1) {
            return result + translateSignular(number, withoutSuffix, key[0], isFuture);
        } else if (withoutSuffix) {
    ...
    function relativeTimeWithSignular(number, withoutSuffix, key) {
    ...
    var lv = _moment__default.defineLocale('lv', {
        ...
        relativeTime : {
            ...
            past : 'pirms %s',
            s : relativeSeconds,
            m : relativeTimeWithSignular,
            mm : lv__relativeTimeWithPlural,
            h : relativeTimeWithSignular,
            hh : lv__relativeTimeWithPlural,
            d : relativeTimeWithSignular,
            dd : lv__relativeTimeWithPlural,
            M : relativeTimeWithSignular,
            MM : lv__relativeTimeWithPlural,
            y : relativeTimeWithSignular,
